    ...
    <div>
        <Row type="flex" class="height-100" :gutter="16">
            <div class="margin-left-10">
                <i-button type="error" @click="goGuide" class="margin-left-10" >使用指南</i-button> 
                <Select v-model="selectBU" @on-change="getDatabaseListNet(selectBU)" style="width:100px" class="margin-left-10">
                    <Option :value="1" :key="1">TBU</Option>
                    <Option :value="2" :key="2">IBU</Option>
                    <Option :value="3" :key="3">NBU</Option>
                    <Option :value="4" :key="4">STP</Option>
                </Select>
...
export default {
    ...
    methods: {
        ...
        showDoc(index) {
            this.$router.push({path:"/cooperation/cooperation-dbdoc-list",query:{id:index,buId:this.selectBU}})
        },
        goGuide() {
            window.open("http://note.youdao.com/noteshare?id=97cdc9ad4c8fe2fed4df35af4660512f")
        },
    },
    // created () {
    //     this.getData()
